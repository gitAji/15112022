import"./listings.js";import"./credits.js";const P=new URLSearchParams(document.location.search),f=P.get("id"),D=localStorage.getItem("accessToken"),g=localStorage.getItem("username"),h="https://nf-api.onrender.com/api/v1",y="/auction/listings/",j="?_seller=true&_bids=true",k=`${h}${y}${f}${j}`,N=document.getElementById("a-post");async function U(n){try{const d=await(await fetch(n,{method:"GET"})).json();console.log(d),_(d,N)}catch(s){console.warn(s)}}U(k);document.getElementById("bid-list");function _(n,s,A){const d=new Date(n.endsAt),B=d.toLocaleString(),L=new Date;document.title=n.title;const a=d-L,T=Math.floor(a/(1e3*60*60*24)),w=Math.floor(a%(1e3*60*60*24)/(1e3*60*60)),$=Math.floor(a%(1e3*60*60)/(1e3*60));var o="";if(a<0?o="Ended":o="Active",o==="Active"){const e=document.getElementById("bidButtonSection");e.innerHTML=`
