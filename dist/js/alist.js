import"./alert.js";import"./credits.js";import"./listings.js";const H=new URLSearchParams(document.location.search),h=H.get("id"),P=localStorage.getItem("accessToken"),v=localStorage.getItem("username"),y="https://nf-api.onrender.com/api/v1",A="/auction/listings/",D="?_seller=true&_bids=true",j=`${y}${A}${h}${D}`,N=document.getElementById("a-post");async function k(n){try{const s=await(await fetch(n,{method:"GET"})).json();console.log(s),U(s,N)}catch(d){console.warn(d)}}k(j);document.getElementById("bid-list");function U(n,d){const r=new Date(n.endsAt),s=r.toLocaleString(),B=new Date;document.title=n.title;const a=r-B,L=Math.floor(a/(1e3*60*60*24)),T=Math.floor(a%(1e3*60*60*24)/(1e3*60*60)),$=Math.floor(a%(1e3*60*60)/(1e3*60));var o="";if(a<0?o="Ended":o="Active",o==="Active"){const e=document.getElementById("bidButtonSection");e.innerHTML=`
    ...
    <button class="btn btn-danger btn-lg" id="bid-button">Bid Ended</button> </div>`;for(let e of n.bids){const c=e.amount,i=Math.max(c),l=document.getElementById("highestBid"),u=`<div class="d-flex  alert alert-light-secondary fw-bold" id="highest">
    Highest bid is ${i}kr</b>  by  ${e.bidderName} &nbsp;<div class="text-muted">Please bid higher than the last bid!</div> </div>`;l.innerHTML=u;const m=new Date(e.created).toLocaleString();e.bidderName;const p=document.getElementById("user-bids");p.innerHTML+=`<div class="d-flex">
