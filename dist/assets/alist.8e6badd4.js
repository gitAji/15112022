import"./nav.635f5b05.js";import"./credits.838e7512.js";const H=new URLSearchParams(document.location.search),h=H.get("id"),P=localStorage.getItem("accessToken"),v=localStorage.getItem("username"),y="https://nf-api.onrender.com/api/v1",A="/auction/listings/",D="?_seller=true&_bids=true",j=`${y}${A}${h}${D}`,N=document.getElementById("a-post");async function k(n){try{const c=await(await fetch(n,{method:"GET"})).json();U(c,N)}catch(d){console.warn(d)}}k(j);document.getElementById("bid-list");function U(n,d){const a=new Date(n.endsAt),c=a.toLocaleString(),B=new Date;document.title=n.title;const i=a-B,L=Math.floor(i/(1e3*60*60*24)),T=Math.floor(i%(1e3*60*60*24)/(1e3*60*60)),$=Math.floor(i%(1e3*60*60)/(1e3*60));var o="";if(i<0?o="Ended":o="Active",o==="Active"){const e=document.getElementById("bidButtonSection");e.innerHTML=`
    ...
    <button class="btn btn-danger btn-lg" id="bid-button">Bid Ended</button> </div>`;for(let e of n.bids){const r=e.amount,s=Math.max(r),b=document.getElementById("highestBid"),u=`<div class="d-flex  alert alert-light-secondary fw-bold" id="highest">
    Highest bid is ${s}kr</b>  by  ${e.bidderName} &nbsp;<div class="text-muted">Please bid higher than the last bid!</div> </div>`;b.innerHTML=u;const m=new Date(e.created).toLocaleString();e.bidderName;const p=document.getElementById("user-bids");p.innerHTML+=`<div class="d-flex">
    ...
    </div>`),l==v&&o==="Active"&&(bidButtonSection.innerHTML=`
    <div class="d-flex justify-content-center col-6 m-auto gap-2">
