const d=new URLSearchParams(document.location.search);d.get("id");const a="https://nf-api.onrender.com/api/v1",r="/auction/listings?limit=12&sort=created&sortOrder=desc",m=`${a}${r}`,o=document.getElementById("all-lists");document.getElementById("post-image");const p=document.getElementById("login"),g=document.getElementById("logout"),u=document.getElementById("myIBuy");let f=[];const h='<a href="login.html">Logout</a>',v=' <a href="profile.html">My iBuy</a>';localStorage.getItem("accessToken")?(g.innerHTML=h,u.innerHTML=v):p.innerHTML='<a href="login.html">Login</a>';async function L(i){try{const s=await(await fetch(i,{method:"GET"})).json();f=s,y(s,o),ownLists(s)}catch{}}L(m);function y(i){o.innerHTML="";let e="";for(let t of i){const n=new Date(t.endsAt).toLocaleString();document.title=t.title;const l=t.media!=""?`${t.media}`:"https://via.placeholder.com/150",c=t.description==null?"This Post has no description":`${t.description}`;e+=` <div class="card col-lg-3 col-md-6 col-sm-12 gap-3 shadow p-3 mb-5 bg-white rounded">
